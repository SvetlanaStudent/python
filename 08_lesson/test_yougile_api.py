...
def test_create_project():
    ...
    response = requests.post(BASE_URL, json=valid_project_payload, headers=HEADERS)
...
def test_get_project_by_id():
    valid_project_payload = {'title': 'Test Project'}
    create_response = requests.post(BASE_URL, json=valid_project_payload, headers=HEADERS)
...
def test_update_project():
    ...
    response = requests.put(f"{BASE_URL}/{project_id}", json=updated_payload, headers=HEADERS)
    assert response.status_code == 200, f"Expected 200, got {response.status_code}"
...
def test_create_project_missing_title():
    invalid_payload = {}
    response = requests.post(BASE_URL, json=invalid_payload, headers=HEADERS)
    assert response.status_code == 400, f"Expected 400, got {response.status_code}"

def test_get_project_by_invalid_id():
    response = requests.get(f"{BASE_URL}/invalid-id", headers=HEADERS)
...
def test_update_project_with_invalid_id():
    ...
    response = requests.put(f"{BASE_URL}/{invalid_project_id}", json=updated_payload, headers=HEADERS)
    assert response.status_code == 404, f"Expected 404, got {response.status_code}"
